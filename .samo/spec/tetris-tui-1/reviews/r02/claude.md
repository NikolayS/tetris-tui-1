...
- (minor) Back-to-Back Tetris scoring is ambiguous. The table row 'Back-to-back Tetris ×1.5' does not state (a) whether the multiplier applies to the base Tetris score (800 × level × 1.5 = 1200 × level) or to some other baseline, (b) what resets the B2B state (any non-tetris clear? any line clear? any lock?), and (c) whether the multiplier applies from the second consecutive Tetris onward or from the first. Scoring parameterized test in §5 mentions 'with/without B2B' but without a precise reset rule the test cases are ambiguous.
...
<!-- samospec:critique v1 -->
{
  "findings": [
    ...
    {
      "category": "ambiguity",
      "text": "Back-to-Back Tetris scoring is ambiguous. The table row 'Back-to-back Tetris ×1.5' does not state (a) whether the multiplier applies to the base Tetris score (800 × level × 1.5 = 1200 × level) or to some other baseline, (b) what resets the B2B state (any non-tetris clear? any line clear? any lock?), and (c) whether the multiplier applies from the second consecutive Tetris onward or from the first. Scoring parameterized test in §5 mentions 'with/without B2B' but without a precise reset rule the test cases are ambiguous.",
      "severity": "minor"
    },
    {
